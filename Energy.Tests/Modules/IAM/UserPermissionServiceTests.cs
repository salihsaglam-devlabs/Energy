...
using Energy.Infrastructure.Modules.IAM.UserPermission.Services;
...
/// <summary>UserPermission CRUD servisi round-trip testi (EF InMemory).</summary>
public sealed class UserPermissionServiceTests
{
    ...
    public async Task List_Works_And_Surrogate_Mutations_NotSupported()
    {
        ...
        var list = await service.GetListAsync(new GetUserPermissionListRequest { PageNumber = 1, PageSize = 10 });
        Assert.True(list.IsSuccess);

        Assert.False((await service.GetByIdAsync(Guid.NewGuid())).IsSuccess);
        Assert.False((await service.UpdateAsync(Guid.NewGuid(), new UpdateUserPermissionRequest())).IsSuccess);
