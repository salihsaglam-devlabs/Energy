...
using Energy.Web.Clients.Core.Branch;

namespace Energy.Web.Controllers.Core;

/// <summary>Branch ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("core/branches")]
public sealed class BranchesController : Controller
{
    private readonly IBranchApiClient _api;

    public BranchesController(IBranchApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Core/Branch/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Core.Branch.Responses.BranchListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
