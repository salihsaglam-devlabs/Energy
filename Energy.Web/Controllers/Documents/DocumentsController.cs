...
using Energy.Web.Clients.Documents.Document;

namespace Energy.Web.Controllers.Documents;

/// <summary>Document ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("documents/documents")]
public sealed class DocumentsController : Controller
{
    private readonly IDocumentApiClient _api;

    public DocumentsController(IDocumentApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Documents/Document/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Documents.Document.Responses.DocumentListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
