...
using Energy.Web.Clients.Modules.Budget.Budget;

namespace Energy.Web.Controllers.Modules.Budget;

/// <summary>Budget ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("budget/budgets")]
public sealed class BudgetsController : Controller
{
    private readonly IBudgetApiClient _api;

    public BudgetsController(IBudgetApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Budget/Budget/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Budget.Budget.Responses.BudgetListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
