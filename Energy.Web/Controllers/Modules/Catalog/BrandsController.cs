...
using Energy.Web.Clients.Modules.Catalog.Brand;

namespace Energy.Web.Controllers.Modules.Catalog;

/// <summary>Brand ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("catalog/brands")]
public sealed class BrandsController : Controller
{
    private readonly IBrandApiClient _api;

    public BrandsController(IBrandApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Catalog/Brand/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Catalog.Brand.Responses.BrandListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
