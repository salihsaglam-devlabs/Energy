...
using Energy.Web.Clients.Modules.Catalog.Material;

namespace Energy.Web.Controllers.Modules.Catalog;

/// <summary>Material ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("catalog/materials")]
public sealed class MaterialsController : Controller
{
    private readonly IMaterialApiClient _api;

    public MaterialsController(IMaterialApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Catalog/Material/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Catalog.Material.Responses.MaterialListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
    ...
    [HttpPut("{id:guid}")]
    [IgnoreAntiforgeryToken]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateMaterialRequest request, CancellationToken ct)
