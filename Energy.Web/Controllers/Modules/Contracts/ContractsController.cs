...
using Energy.Web.Clients.Modules.Contracts.Contract;

namespace Energy.Web.Controllers.Modules.Contracts;

/// <summary>Contract ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("contracts/contracts")]
public sealed class ContractsController : Controller
{
    private readonly IContractApiClient _api;

    public ContractsController(IContractApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Contracts/Contract/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Contracts.Contract.Responses.ContractListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
