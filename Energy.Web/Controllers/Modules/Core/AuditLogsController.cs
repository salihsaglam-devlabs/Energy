...
using Energy.Web.Clients.Modules.Core.AuditLog;

namespace Energy.Web.Controllers.Modules.Core;

/// <summary>AuditLog ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("core/audit-logs")]
public sealed class AuditLogsController : Controller
{
    private readonly IAuditLogApiClient _api;

    public AuditLogsController(IAuditLogApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Core/AuditLog/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Core.AuditLog.Responses.AuditLogListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
    ...
    [HttpPut("{id:guid}")]
    [IgnoreAntiforgeryToken]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateAuditLogRequest request, CancellationToken ct)
