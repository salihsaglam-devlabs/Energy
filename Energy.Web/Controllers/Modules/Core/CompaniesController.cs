...
using Energy.Web.Clients.Modules.Core.Company;

namespace Energy.Web.Controllers.Modules.Core;

/// <summary>Company ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("core/companies")]
public sealed class CompaniesController : Controller
{
    private readonly ICompanyApiClient _api;

    public CompaniesController(ICompanyApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Core/Company/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Core.Company.Responses.CompanyListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
