...
using Energy.Web.Clients.Modules.Core.Currency;

namespace Energy.Web.Controllers.Modules.Core;

/// <summary>Currency ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
...
public sealed class CurrenciesController : Controller
{
    ...
    public CurrenciesController(ICurrencyApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Core/Currency/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Core.Currency.Responses.CurrencyListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
