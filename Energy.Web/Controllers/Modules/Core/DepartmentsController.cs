...
using Energy.Web.Clients.Modules.Core.Department;

namespace Energy.Web.Controllers.Modules.Core;

/// <summary>Department ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("core/departments")]
public sealed class DepartmentsController : Controller
{
    private readonly IDepartmentApiClient _api;

    public DepartmentsController(IDepartmentApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Core/Department/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Core.Department.Responses.DepartmentListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
