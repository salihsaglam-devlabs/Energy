...
using Energy.Web.Clients.Modules.Core.UnitOfMeasure;

namespace Energy.Web.Controllers.Modules.Core;

/// <summary>UnitOfMeasure ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("core/units-of-measure")]
public sealed class UnitsOfMeasureController : Controller
{
    private readonly IUnitOfMeasureApiClient _api;

    public UnitsOfMeasureController(IUnitOfMeasureApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Core/UnitOfMeasure/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Core.UnitOfMeasure.Responses.UnitOfMeasureListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
    ...
    [HttpPut("{id:guid}")]
    [IgnoreAntiforgeryToken]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateUnitOfMeasureRequest request, CancellationToken ct)
