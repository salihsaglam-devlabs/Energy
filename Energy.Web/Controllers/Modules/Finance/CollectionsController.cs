...
using Energy.Web.Clients.Modules.Finance.Collection;

namespace Energy.Web.Controllers.Modules.Finance;

/// <summary>Collection ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("finance/collections")]
public sealed class CollectionsController : Controller
{
    private readonly ICollectionApiClient _api;

    public CollectionsController(ICollectionApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Finance/Collection/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Finance.Collection.Responses.CollectionListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
