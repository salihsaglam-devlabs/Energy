...
using Energy.Web.Clients.Modules.Finance.FinancialAccount;

namespace Energy.Web.Controllers.Modules.Finance;

/// <summary>FinancialAccount ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("finance/financial-accounts")]
public sealed class FinancialAccountsController : Controller
{
    private readonly IFinancialAccountApiClient _api;

    public FinancialAccountsController(IFinancialAccountApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Finance/FinancialAccount/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Finance.FinancialAccount.Responses.FinancialAccountListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
