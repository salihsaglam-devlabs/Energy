...
using Energy.Web.Clients.Modules.HR.Timesheet;

namespace Energy.Web.Controllers.Modules.HR;

/// <summary>Timesheet ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("h-r/timesheets")]
public sealed class TimesheetsController : Controller
{
    private readonly ITimesheetApiClient _api;

    public TimesheetsController(ITimesheetApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/HR/Timesheet/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.HR.Timesheet.Responses.TimesheetListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
