...
using Energy.Web.Clients.Modules.Inventory.StockCount;

namespace Energy.Web.Controllers.Modules.Inventory;

/// <summary>StockCount ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("inventory/stock-counts")]
public sealed class StockCountsController : Controller
{
    private readonly IStockCountApiClient _api;

    public StockCountsController(IStockCountApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Inventory/StockCount/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Inventory.StockCount.Responses.StockCountListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
    ...
    [HttpPut("{id:guid}")]
    [IgnoreAntiforgeryToken]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateStockCountRequest request, CancellationToken ct)
