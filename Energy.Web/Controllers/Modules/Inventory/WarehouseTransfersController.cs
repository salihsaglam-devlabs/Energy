...
using Energy.Web.Clients.Modules.Inventory.WarehouseTransfer;

namespace Energy.Web.Controllers.Modules.Inventory;

/// <summary>WarehouseTransfer ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("inventory/warehouse-transfers")]
public sealed class WarehouseTransfersController : Controller
{
    private readonly IWarehouseTransferApiClient _api;

    public WarehouseTransfersController(IWarehouseTransferApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Inventory/WarehouseTransfer/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Inventory.WarehouseTransfer.Responses.WarehouseTransferListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
    ...
    [HttpPut("{id:guid}")]
    [IgnoreAntiforgeryToken]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateWarehouseTransferRequest request, CancellationToken ct)
