...
using Energy.Web.Clients.Modules.Inventory.Warehouse;

namespace Energy.Web.Controllers.Modules.Inventory;

/// <summary>Warehouse ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
...
public sealed class WarehousesController : Controller
{
    ...
    public WarehousesController(IWarehouseApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Inventory/Warehouse/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Inventory.Warehouse.Responses.WarehouseListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
