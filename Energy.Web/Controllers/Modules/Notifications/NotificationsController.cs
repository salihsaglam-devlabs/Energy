...
using Energy.Web.Clients.Modules.Notifications.Notification;

namespace Energy.Web.Controllers.Modules.Notifications;

/// <summary>Notification ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
...
public sealed class NotificationsController : Controller
{
    ...
    public NotificationsController(INotificationApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Notifications/Notification/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Notifications.Notification.Responses.NotificationListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
