...
using Energy.Web.Clients.Modules.Operations.WorkOrder;

namespace Energy.Web.Controllers.Modules.Operations;

/// <summary>WorkOrder ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("operations/work-orders")]
public sealed class WorkOrdersController : Controller
{
    private readonly IWorkOrderApiClient _api;

    public WorkOrdersController(IWorkOrderApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Operations/WorkOrder/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Operations.WorkOrder.Responses.WorkOrderListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
