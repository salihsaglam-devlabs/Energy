...
using Energy.Web.Clients.Modules.Organization.Employee;

namespace Energy.Web.Controllers.Modules.Organization;

/// <summary>Employee ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
...
public sealed class EmployeesController : Controller
{
    ...
    public EmployeesController(IEmployeeApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Organization/Employee/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Organization.Employee.Responses.EmployeeListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
