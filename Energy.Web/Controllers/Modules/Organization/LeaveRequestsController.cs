...
using Energy.Web.Clients.Modules.Organization.LeaveRequest;

namespace Energy.Web.Controllers.Modules.Organization;

/// <summary>LeaveRequest ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("organization/leave-requests")]
public sealed class LeaveRequestsController : Controller
{
    private readonly ILeaveRequestApiClient _api;

    public LeaveRequestsController(ILeaveRequestApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Modules/Organization/LeaveRequest/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Organization.LeaveRequest.Responses.LeaveRequestListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
    ...
    [HttpPut("{id:guid}")]
    [IgnoreAntiforgeryToken]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateLeaveRequestRequest request, CancellationToken ct)
