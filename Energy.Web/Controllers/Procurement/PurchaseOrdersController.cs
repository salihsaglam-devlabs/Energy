...
using Energy.Web.Clients.Procurement.PurchaseOrder;

namespace Energy.Web.Controllers.Procurement;

/// <summary>PurchaseOrder ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
...
public sealed class PurchaseOrdersController : Controller
{
    ...
    public PurchaseOrdersController(IPurchaseOrderApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Procurement/PurchaseOrder/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Procurement.PurchaseOrder.Responses.PurchaseOrderListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
