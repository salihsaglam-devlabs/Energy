...
using Energy.Web.Clients.Procurement.SupplierInvoice;

namespace Energy.Web.Controllers.Procurement;

/// <summary>SupplierInvoice ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("procurement/supplier-invoices")]
public sealed class SupplierInvoicesController : Controller
{
    private readonly ISupplierInvoiceApiClient _api;

    public SupplierInvoicesController(ISupplierInvoiceApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Procurement/SupplierInvoice/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Procurement.SupplierInvoice.Responses.SupplierInvoiceListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
    ...
    [HttpPut("{id:guid}")]
    [IgnoreAntiforgeryToken]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateSupplierInvoiceRequest request, CancellationToken ct)
