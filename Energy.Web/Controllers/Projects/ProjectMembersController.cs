...
using Energy.Web.Clients.Projects.ProjectMember;

namespace Energy.Web.Controllers.Projects;

/// <summary>ProjectMember ekran denetleyicisi (yalnızca API istemcisiyle konuşur).</summary>
[Authorize]
[Route("projects/project-members")]
public sealed class ProjectMembersController : Controller
{
    private readonly IProjectMemberApiClient _api;

    public ProjectMembersController(IProjectMemberApiClient api) => _api = api;

    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Projects/ProjectMember/Index.cshtml");
    ...
    public async Task<IActionResult> List(int skip = 0, int take = 20, string? searchValue = null, CancellationToken ct = default)
    {
        ...
        return Json(new { data = page?.Items ?? Array.Empty<Energy.Shared.Models.V1.Projects.ProjectMember.Responses.ProjectMemberListResponse>(), totalCount = page?.TotalCount ?? 0 });
    }
    ...
    [HttpPut("{id:guid}")]
    [IgnoreAntiforgeryToken]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateProjectMemberRequest request, CancellationToken ct)
